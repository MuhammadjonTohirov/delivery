...
MENU_ITEMS_PER_CATEGORY = [5, 8]  # Min and max menu items per category
...
def create_order(customer, restaurant, menu_items):
    ...
    order_data = {
        ...
        "delivery_lat": float(fake.latitude()),
        "delivery_lng": float(fake.longitude()),
        "delivery_fee": "2.50",
...
def process_order(order, restaurant_owner, driver=None):
    ...
    response = make_request(
        'post', 
        f"orders/{order['id']}/ready_for_pickup/", 
    ...
    if driver and driver['token']:
        ...
        task_data = {
            "driver": driver['data']['driver_profile']['id'],
            "order": order['id'],
            "status": "PENDING",
        ...
        # Driver accepts task
        response = make_request('post', f"drivers/tasks/{task_id}/accept/", {}, driver['token'])
        ...
        response = make_request('post', f"drivers/tasks/{task_id}/picked_up/", {}, driver['token'])
        ...
        response = make_request('post', f"drivers/tasks/{task_id}/delivered/", {}, driver['token'])
